...
def check_new_user(login: str, email: str, password: str) -> bool:
    ...
    pattern_login - Эта регулярка проверяет, соответствует ли строка следующим условиям:
    Длина строки от 5 до 25 символов.
    Строка может содержать только латинские буквы в верхнем и нижнем регистре, цифры, а также символы !, ?,.
    ...
    состоящая из букв, цифр, дефисов и точек, где точка не может быть первым или последним символом,
    и между точками должен быть хотя бы один символ.
    ...
    То есть, данное регулярное выражение подходит для проверки корректности пароля,
    который должен содержать только разрешенные символы и быть длиной не менее 5 и не более 36 символов.
